 ...
 */
function network_init_members() {

	//Members
  register_post_type( 'network_member',
    array(
      'labels' => array(
        ...
        'parent_item_colon' => '',
        'menu_name' => __('Network Members')
      ),
    'menu_position' => 21,
    'show_ui' => true,
    'description' => 'Network Member publications/groups',
...
add_action( 'init', 'network_init_members', 11 );
  ...
  $meta_box = array(
    ...
    'fields' => array(
      ...
      array(
        ...
        'type'  => 'text',
        'size'	=> 5,
        'desc'   => "The year this group joined the network, if known."
      ),
...
add_action( 'admin_init', 'network_meta_boxes' );


/**
 * Geocode the coordinates for members when edited
 */
function network_geocode_member( $post_id ) {
	// Don't do anything if we're not supposed to
	// Might use $post->post_type instead of $_POST?
	if ( 'network_member' != get_post_type( $post_id ) || !current_user_can( 'edit_post', $post_id ) || !strlen(trim( $_POST['network_address'] )) ) return;
	if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ) return;

	// Get & format the entered address
	$post_address = urlencode( $_POST['network_address'] );
	...
	if ( $result->status == 'OK' ) {
		...
		update_post_meta( $post_id, 'network_coords', $coords );
	} else {
 ...
 */
function network_display_errors() {
...
add_action( 'admin_notices', 'network_display_errors', 20 );
	...
	<script type="text/javascript">
		...
		<?php
		 foreach ( $members as $member ) :
		 	...
		 	if ( !isset($member->network_coords) || empty($member->network_coords)) continue;
		 	$info = sprintf('<div class="map-popup"><a href="%s" class="map-name">%s</a><br/><a href="%s" target="_blank">%s</a></div>',
		 		...
		 		$member->network_site_url,
		 		$member->network_site_url
		 	);
			 ?>{
title: "X<?php echo htmlspecialchars($member->post_title, ENT_QUOTES); ?>",
latLng: new gm.LatLng(<?php echo $member->network_coords ?>),
d: '<?php echo $info; ?>'
},<?php
		 endforeach;
		?>
 ...
 * Alphabetical links
 */
function network_member_alpha_links() {
 ...
 */
function network_members_by_letter( $query ) {

  //get members by letter
  if ( $query->is_post_type_archive('network_member') && $query->is_main_query() && isset($_GET['letter']) ) {
  	$query->set( 'title_starts_with', $_GET['letter'] );
  	$query->set( 'posts_per_page', -1 );
  }

	//order members by name
  if ( $query->is_post_type_archive('network_member') && $query->is_main_query()  ) {
...
add_action( 'pre_get_posts', 'network_members_by_letter' );
 ...
 */
function network_disable_member_redirect( $redirect_url ) {
	if (is_singular('network_member')) $redirect_url = false;
	return $redirect_url;
}
add_filter('redirect_canonical', 'network_disable_member_redirect');