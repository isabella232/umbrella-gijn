			...
			<article id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?>>
				...
				<nav class="guide-nav span3">
						<?php if ( $top_page ) { ?>
							<h4>Contents</h4>
						<?php } else { ?>
							<h4 class="guide-top"><a href="<?php echo get_permalink($ancestors[0]); ?>"><?php echo get_the_title($ancestors[0]); ?></a></h4>
						<?php } ?>
						<ul>
							<?php echo $children; ?>
						</ul>
				...
				<div class="entry-content span9">
					...
					<nav id="nav-below" class="pager post-nav clearfix">
						<?php
							$pagelist = get_pages('sort_column=menu_order&sort_order=asc&child_of='.$ancestors[0]);
							$pages = array();
